...
#ifdef TRISOS  /* avoid segment for TCP_NODELAY*/
    struct msghdr msg;
    struct iovec io[4];
    memset(&msg, 0, sizeof(msghdr));
    io[0].iov_base = (void *)&dollar;
    io[0].iov_len = 1;
    io[1].iov_base = (void *)&streamChannelId;
    io[1].iov_len = 1;
    io[2].iov_base = (void *)netPacketSize;
    io[2].iov_len = 2;
    io[3].iov_base = (void *)packet;
    io[3].iov_len = packetSize;
    msg.msg_iov = io;
    msg.msg_iovlen = 4;
    if (sendmsg(socketNum, &msg, 0) != (int)(1+1+2+packetSize)) break;
#endif
...
#ifdef TRISOS
  fprintf(stderr, "sendRTPOverTCP: failed (%s)\n",strerror(errno)); fflush(stderr);
