...
#ifdef TRISOS	  
	  else if (strncmp(valueStr, "reso=",5) == 0) {
    	    fVideoWidth = (unsigned short)atoi(valueStr+5);
    	    fVideoHeight = (unsigned short)atoi(strstr(valueStr,":")+1);
	  }
#endif	
	}
      }
...
#ifdef TRISOS
  else if (sscanf(sdpLine, "a=cliprect:%d,%d,%d,%d", &sheight, &swidth, &height, &width) == 4) {
    parseSuccess = True;
    fVideoWidth = (unsigned short)(width - swidth);
    fVideoHeight = (unsigned short)(height - sheight);
  }
#endif
...
#ifdef TRISOS
  else if (sscanf(sdpLine, "a=framrate: %f", &frate) == 1 || sscanf(sdpLine, "a=framrate:%f", &frate) == 1) {
    parseSuccess = True;
    fVideoFPS = (unsigned)frate;
  }
#endif
