...
int RTSPServer::setUpOurSocket(UsageEnvironment& env, Port& ourPort) {
  ...
  do {
#ifndef TRISOS  /* reuse the address & port for trisos*/
...
#ifdef TRISOS 
    if(socket_keep_alive(ourSocket)) {
        env.setResultMsg("socket_keep_alive() failed: ", strerror(errno));
        break;
    }    
#endif
...
#ifdef TRISOS
      /* don't remove this session again,
         because if fOurServerMediaSession->deleteWhenUnreferenced() is true, that means this session has already
         been removed from server's hash table. if we remove again, that would remove the new session with the save name
         */
