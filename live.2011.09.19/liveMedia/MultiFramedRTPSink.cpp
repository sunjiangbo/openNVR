...
#ifdef TRISOS
unsigned MultiFramedRTPSink::fDefaultKbitsPerMS = 0;
#endif
...
void MultiFramedRTPSink::sendPacketIfNecessary() {
...
#ifdef TRISOS
    if(fDefaultKbitsPerMS) {
        packetSize = fOutBuf->curPacketSize();
    }
#endif
  }
  ...
  if (fNoFramesLeft) {
    ...
    if(fDefaultKbitsPerMS) {
      int64_t curMS;
      curMS = timeNow.tv_sec * 1000 + timeNow.tv_usec / 1000;
      
      if(curMS != fLastMS) {
        fLastMS = curMS;
        fHasSentBytes = packetSize;
      }else{
        fHasSentBytes += packetSize;  
      }

      if(fHasSentBytes * 8 > (fDefaultKbitsPerMS * 1024)) {
        unsigned leftUsec = 1000 - timeNow.tv_usec % 1000;
        if(uSecondsToGo < leftUsec) {
            uSecondsToGo = leftUsec;
        }
      }        
