...
#ifdef TRISOS
int videoSyncReady = 1;
int audioSyncReady = 1;
#endif
////////// RTCPMemberDatabase //////////
...
#ifdef TRISOS
  /**
   Hack: for Trisos, don't send SR until our rtsp client has seen
   the RTCP syn packet
  */
  if(fSink != NULL){
       
      if(strcmp(fSink->sdpMediaType(), "video") == 0  && videoSyncReady == 0) {
          return;
      }else if(strcmp(fSink->sdpMediaType(), "audio") == 0 && audioSyncReady == 0){
          return;
      }
      if(fSink->source() != NULL) {
          struct timeval timeNow;
          timeNow = fSink->source()->lastSeenPresentationTime();
          if(timeNow.tv_sec == 0 && timeNow.tv_usec == 0) {
              return; /* not receive any packet */
          }
      }
...
void RTCPInstance::addSR() {
  ...
  gettimeofday(&timeNow, NULL);
#else
  /*don't use the current time in SR, because if the current time is far from the packet's 
    presenation time, the rtp timestamp gap would be overflow the u32 range, which would cause
    receiver make a wrong time sync.
    instead, use the last presenation time the packet is more safe */
  if(fSink->source() != NULL) {
    timeNow = fSink->source()->lastSeenPresentationTime();
  }else{
    gettimeofday(&timeNow, NULL);
